    ...
    const handleGameURL = async () => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/games/4e375a20-f793-44e1-a9ef-f140dc5ea94a/serve`,
          {
            method: "GET",
            headers: {
        ...
        if (response.ok && data.ip && data.port) {
          const url = `http://${data.ip}:${data.port}/Final Destiny`
